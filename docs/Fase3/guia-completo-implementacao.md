...
Este guia detalha os passos para implementar infraestrutura imutável, DevSecOps e GitOps para o ToggleMaster usando IaC, CI/CD e ArgoCD.

**Nota:** Todos os blocos de código marcados com ```bash devem ser executados no terminal. Eles estão destacados com "**Cole no terminal:**" para facilitar a identificação.

---

## Desafio

A DevOps Solutions Inc. aprovou a arquitetura, mas a operação é insustentável. Precisamos de "Se não está no código, não existe": IaC, pipelines de segurança e GitOps.

## Nota Importante: Ambiente AWS Academy

- **Não pode criar Roles ou Policies IAM via Terraform**
- Use a LabRole existente para EKS e Node Groups
- Importe a LabRole via data source ou variável

## Pré-requisitos de Instalação

### Script Automático de Instalação (Recomendado)

Existe um script PowerShell que instala todas as ferramentas automaticamente.

**Cole no terminal PowerShell (como Administrador):**
...
**Feche o terminal e abra um novo para aplicar as mudanças de PATH!**

### Instalação Manual (Se preferir)

#### Terraform

**Cole no terminal (Windows):**

```bash
# Opção 1: Usando Chocolatey (se tiver instalado)
...
# Acesse: https://www.terraform.io/downloads.html
# Extraia o arquivo e adicione à PATH
```
...
```
Infraestrutura Imutável (Terraform)
├── VPC + Subnets + IGW
├── EKS Cluster (LabRole)
├── RDS PostgreSQL x3
├── ElastiCache Redis
...
| ----- | --------- | ----------- |
| 0 | Pré-requisitos | Terraform, AWS CLI, kubectl, Helm |
| 1 | IaC com Terraform | VPC, EKS, Bancos, SQS |
| 2 | CI/CD DevSecOps | GitHub Actions, Security Scans |
| 3 | GitOps com ArgoCD | Manifests repo, Auto-sync |
| 4 | Testes e Validação | Pipeline + ArgoCD |
| 5 | Entregáveis | Vídeo, Código, Relatório |

---

## ETAPA 0: Pré-requisitos e Configuração Inicial

### 0.1. Verificar Instalações

Após instalar Terraform, AWS CLI, kubectl e Helm, verifique se todas as ferramentas estão funcionando:
...
**Saída esperada:**
```
version.BuildInfo{Version:"v3.13.0", GitCommit:"...", GitTreeState:"clean", GoVersion:"..."}
```

**Se alguma ferramenta não for encontrada, execute o script de instalação:**
...
### 0.3. Criar Bucket S3 para Backend Terraform
...
aws dynamodb create-table \
  --table-name togglemaster-tfstate-lock \
...
**⚠️ IMPORTANTE:** Altere `seu-bucket-terraform-state` para um nome único (ex: `togglemaster-tfstate-seu-email`)

### 0.4. Atualizar Backend Terraform

Edite o arquivo `terraform/backend.tf` e atualize o nome do bucket:

```hcl
terraform {
  backend "s3" {
    bucket         = "seu-bucket-terraform-state"  # Mude isso
...
## ETAPA 1: Infraestrutura como Código (Terraform)
...
### 1.4. EKS Cluster (AWS Academy)

```hcl
data "aws_iam_role" "lab_role" {
  name = "LabRole"  # Importar LabRole existente
}
...
## ETAPA 2: Pipeline CI/CD DevSecOps

### 2.1. GitHub Actions Workflow

Crie `.github/workflows/ci.yml` para cada microsserviço:
...
jobs:
  ...
  docker:
    ...
    - name: Push to ECR
      run: |
        ECR_URI=${{ steps.login-ecr.outputs.registry }}/${{ github.repository }}
        docker tag ${{ github.sha }} $ECR_URI:${{ github.sha }}
        docker push $ECR_URI:${{ github.sha }}
    
    - name: Update GitOps repo
      run: |
        # Script para atualizar tag no repo GitOps
        ./scripts/update-gitops.sh ${{ github.sha }}
```

### 2.2. Configuração para Python Services

Para serviços Python, ajuste o workflow:
...
- name: Run pylint
  run: pylint $(find . -name "*.py" -not -path "./tests/*")
...
## ETAPA 3: GitOps com ArgoCD

### 3.1. Repositório GitOps

Crie um repo separado `togglemaster-gitops` com:
...
spec:
  ...
  source:
    repoURL: https://github.com/YOUR_ORG/togglemaster-gitops
    targetRevision: HEAD
    path: manifests/deployments/auth-service
  destination:
...
# Clone repo gitops
git clone https://github.com/YOUR_ORG/togglemaster-gitops.git
cd togglemaster-gitops

# Update deployment.yaml
sed -i "s|image:.*|image: YOUR_ECR_URI/$SERVICE:$NEW_TAG|g" manifests/deployments/$SERVICE/deployment.yaml

# Commit and push
git add .
git commit -m "Update $SERVICE to $NEW_TAG"
git push
```
...
### 4.1. Validar Infraestrutura

**Cole no terminal:**

```bash
terraform plan  # Deve mostrar no changes
kubectl get nodes  # Nodes ativos
kubectl get pods -n togglemaster  # Aplicações rodando
```

### 4.2. Testar Pipeline

1. Faça commit com vulnerabilidade intencional
2. Pipeline deve falhar no security scan
3. Corrija e veja passar
4. Verifique imagem no ECR

### 4.3. Testar GitOps

1. Pipeline atualiza tag no GitOps repo
2. ArgoCD detecta mudança
3. Sync automático no cluster
4. Verifique nova versão rodando
...
- **IaC**: `terraform plan` e `apply`
- **DevSecOps**: Pipeline falhando/correndo com erro intencional
- **GitOps**: Update automático no repo GitOps
- **ArgoCD**: Sync automático das aplicações

### Código Fonte

- `terraform/` - IaC completo
- `.github/workflows/` - Pipelines CI/CD
- `gitops/` - Manifests para ArgoCD

### Relatório

- Nomes dos participantes
- Links do vídeo e docs
- Desafios e decisões
- Print custos AWS
...
- **AWS Academy**: Sempre use LabRole, não crie IAM
- **ArgoCD**: Verifique RBAC e permissões
- **Pipeline**: Configure secrets AWS no GitHub
- **Terraform State**: Use S3 backend com lock

## Roteiro do Vídeo

1. **IaC**: Demo terraform apply criando recursos
2. **Pipeline**: Trigger manual com erro de segurança
3. **Correção**: Fix e re-run passando
4. **GitOps**: ArgoCD interface mostrando sync
5. **Custos**: Estimativa AWS Academy
